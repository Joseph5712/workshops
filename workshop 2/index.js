const express = require("express");//es un framework de Node.js para construir aplicaciones web y APIs
const cors = require("cors");//mecanismo que permite a las aplicaciones web de un dominio hacer solicitudes a un servidor en un dominio diferente
const bodyParser = require("body-parser");//analizar el cuerpo de las solicitudes HTTP entrantes
const app = express();//Esto crea una instancia de una aplicación Express. Esta instancia es la que se utilizará para definir rutas y middleware para manejar las solicitudes HTTP
...
app.get("/", function (req, res) {
  const response = [{ response: "Workshop #2" }];
  res.json(response);
});
...
app.get("/num2/hello", function (req, res) {
  ...
  const response = [{ response: `Hello ${message}` }];
  res.json(response);
});
